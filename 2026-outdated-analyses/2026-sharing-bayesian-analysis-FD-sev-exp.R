...
#-------------------------------------------------------------------------------
data_clean_S3 <- ds_FA[complete.cases(ds_FA[, c("allhazards_H9a_sev_exposure_30_IA", "S3_Resolved_IA_TR")]), ]

# quasi complete separation...
ggplot(data_clean_S3, aes(x = allhazards_H9a_sev_exposure_30_IA, y = S3_Resolved_IA_TR)) +
  geom_jitter(height = 0.05, alpha = 0.6) +
  geom_smooth(method = "loess", se = FALSE, color = "#66c2a4")

data_clean_S3 %>%
  mutate(bin = cut(allhazards_H9a_sev_exposure_30, breaks = seq(-2, 2, 0.5))) %>%
  group_by(bin) %>%
  summarise(mean_outcome = mean(S3_Resolved_IA_TR), n = n())

mod_S3 <- brm(
  formula = as.factor(S3_Resolved_IA_TR) ~ allhazards_H9a_sev_exposure_30_IA,
  data = data_clean_S3,
...
mod_glm <- glm(
  as.factor(S3_Resolved_IA_TR) ~ allhazards_H9a_sev_exposure_30_IA,
  data = data_clean_S3,
...
pp_S3 <- pp_check(mod_S3, type = "bars", ndraws = 200) +
  ...
  labs(x = "Food Destroying Hazard Exposure", y = "Density")

pp_S3

pp_check(mod_S3, type='error_scatter_avg', ndraws = 200)
hypothesis(mod_S3, "allhazards_H9a_sev_exposure_30_IA = 0")
...
hypothesis(mod_S3, "allhazards_H9a_sev_exposure_30_IA > 0") # they do!
